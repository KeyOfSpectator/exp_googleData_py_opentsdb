...
def execute_put_tsdb_Attr(row , metric , index_of_row ,TSDB_IP , time_stamp):
    """
    ...
    try:
        ...
        put_str = "%s %s %s start_time=%s end_time=%s jobID=%s task_index=%s machineID=%s" % (metric, time_stamp , row[index_of_row] , row[0] , row[1] , row[2] , row[3] , row[4])
...
#     print threading.currentThread().name, 'start at', get_current_time()
    ...
    try:
        while row is not None:
            ...
            #execute(db, cur, row)
            execute_put_tsdb_Attr(row , 'google.data.usage.CPU_rate' , 5 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.canonical_memory_usage' , 6 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.assigned_memory_usage' , 7 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.unmapped_page_cache' , 8 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.total_page_cache' , 9 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.maximum_memory_usage' , 10 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.disk_IO_time' , 11 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.local_disk_space_usage' , 12 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.maximum_CPU_rate' , 13 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.maximum_disk_IO_time' , 14 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.cycles_per_instruction' , 15 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.memory_accesses_per_instruction' , 16 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.sample_portion' , 17 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.aggregation_type' , 18 , TSDB_IP , time_stamp)
            execute_put_tsdb_Attr(row , 'google.data.usage.sampled_CPU_usage' , 19 , TSDB_IP , time_stamp)
